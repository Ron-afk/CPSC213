...
static void *coalesce(struct myheap *h, void *first_block_start) {
  ...
  void *coalesced_block_start = first_block_start;
  if (!is_last_block(h, first_block_start)) {
    void *second_block_start = get_next_block(first_block_start);
    while (!block_is_in_use(coalesced_block_start) && is_within_heap_range(h,second_block_start) 
    && !block_is_in_use(second_block_start)) {
      int total_size = get_block_size(coalesced_block_start) + get_block_size(second_block_start);
      set_block_header(first_block_start, total_size, 0);
      second_block_start = get_next_block(coalesced_block_start);
    }
  }
  return coalesced_block_start;
}
...
static int get_size_to_allocate(int user_size) {
  ...
  /* TO BE COMPLETED BY THE STUDENT. */
  // int temp = user_size;
  // if (temp % HEADER_SIZE != 0){
  //   temp = ((temp % HEADER_SIZE) + 1)*HEADER_SIZE;
  // }
  // int result = temp + 2*HEADER_SIZE;
  // return result;
  int size = user_size + 2 * HEADER_SIZE;
  int padding = 0;
  if(user_size % HEADER_SIZE != 0) {
    padding = HEADER_SIZE - user_size % HEADER_SIZE;
  }
  size += padding;
  return size;
...
static void *split_and_mark_used(struct myheap *h, void *block_start, int needed_size) {
  ...
  int left = current_block_size - needed_size;
  set_block_header(block_start,current_block_size,1);
 
  if (left >= 3*HEADER_SIZE) {
    set_block_header(block_start,needed_size,1);
    void *second_start = get_next_block(block_start);
    set_block_header(second_start,left,0);
    coalesce(h, second_start);
  // } else {
  //   set_block_header(block_start,current_block_size,1);
  }
  void* payload = get_payload(block_start);
  return payload;
}
...
void myheap_free(struct myheap *h, void *payload) {
  ...
  /* TO BE COMPLETED BY THE STUDENT. */
  void *current_block_start = get_block_start(payload);
  int current_block_size = get_block_size(current_block_start);
  set_block_header(current_block_start,current_block_size,0);
  current_block_start = coalesce(h, current_block_start);

  if (!is_first_block(h, current_block_start)){
    void *previous = get_previous_block(current_block_start);
    coalesce(h, previous);
  }
}
...
void *myheap_malloc(struct myheap *h, unsigned int user_size) {
  ...
  /* TO BE COMPLETED BY THE STUDENT. */
  int required_size = get_size_to_allocate(user_size);
  void *payload;
  for (void *blk = h->start; is_within_heap_range(h, blk); blk = get_next_block(blk)) {
    int current_size = get_block_size(blk);
    if (!block_is_in_use(blk) && current_size >= required_size) {
      payload = split_and_mark_used(h,blk,required_size);
      return payload;
    }
  }
  return NULL;
